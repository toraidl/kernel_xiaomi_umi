...
static int __bq25790_read_reg(struct bq25790 *bq, u8 reg, u8 *data)
{
	...
	if (ret < 0) {
		bq_dbg(PR_OEM,
		       "i2c read byte fail: can't read from reg 0x%02X\n", reg);
		return ret;
	}
...
static int bq25790_update_byte_bits(struct bq25790 *bq, u8 reg, u8 mask,
				    u8 data)
{
	int ret;
	u8 tmp;
...
static int bq25790_update_word_bits(struct bq25790 *bq, u8 reg, u16 mask,
				    u16 data)
{
	int ret;
...
static int bq25790_set_arti_vbus_disable(struct bq25790 *bq, bool disable)
{
	...
	bq_dbg(PR_OEM,
	       "set arti vbus disable to:%d   bq->arti_vbus_enable:%d\n",
...
static int bq25790_set_adc_simple(struct bq25790 *bq, int simple)
{
	...
	ret = bq25790_update_byte_bits(bq, BQ25790_ADC_CONTROL_REG,
				       BQ25790_ADC_SAMPLE_MASK,
				       simple << BQ25790_ADC_SAMPLE_SHFIT);
...
static int bq25790_set_adc_dis_mask(struct bq25790 *bq)
{
	...
	ret = bq25790_update_byte_bits(bq, BQ25790_REG_ADC_CTRL_REG,
				       BQ25790_ADC_EN_MASK,
				       BQ25790_ADC_EN << BQ25790_ADC_EN_SHIFT);
	if (ret)
		bq_dbg(PR_OEM, "failed to set adc enable\n");

	reg_val = TS_ADC_DIS | TDIE_ADC_DIS;

	ret = bq25790_update_byte_bits(bq, BQ25790_REG_ADC_DISABLE_REG, 0xFF,
				       reg_val);
	if (ret)
		bq_dbg(PR_OEM, "failed to set adc enable\n");
...
static int bq25790_set_int_mask(struct bq25790 *bq)
{
	...
	reg_val =
		AC1_PRESENT_MASK | AC2_PRESENT_MASK | VINDPM_MASK | IINDPM_MASK;
	ret = bq25790_update_byte_bits(bq, BQ25790_REG_CHG_INT_MASK0, 0xFF,
				       reg_val);
	if (ret)
		return ret;

	reg_val = BC12_DONE_MASK;
	ret = bq25790_update_byte_bits(bq, BQ25790_REG_CHG_INT_MASK1, 0xFF,
				       reg_val);
	if (ret)
		return ret;
	...
	ret = bq25790_update_byte_bits(bq, BQ25790_REG_CHG_INT_MASK2, 0xFF,
				       reg_val);
	if (ret)
		return ret;
...
static int bq25790_set_fault_mask(struct bq25790 *bq)
{
	...
	ret = bq25790_update_byte_bits(bq, BQ25790_REG_CHG_FAULT_MASK0, 0xFF,
				       reg_val);
	if (ret)
		return ret;

	reg_val = TSHUT_MASK | OTG_UVP_MASK | OTG_OVP_MASK;
	ret = bq25790_update_byte_bits(bq, BQ25790_REG_CHG_FAULT_MASK1, 0xFF,
				       reg_val);
	if (ret)
		return ret;
...
static int bq25790_get_prop_charge_type(struct bq25790 *bq)
{
	...
	bq25790_get_property_from_bms(bq, POWER_SUPPLY_PROP_VOLTAGE_NOW,
				      &batt_prop);
...
static int bq25790_get_prop_charge_status(struct bq25790 *bq)
{
	...
	ret = bq25790_get_property_from_bms(bq, POWER_SUPPLY_PROP_STATUS,
					    &batt_prop);
	...
	bq->charge_state =
		(status & BQ25790_CHRG_STAT_MASK) >> BQ25790_CHRG_STAT_SHIFT;
...
static int bq25790_get_prop_health(struct bq25790 *bq)
{
	...
	ret = bq25790_get_property_from_bms(bq, POWER_SUPPLY_PROP_HEALTH,
					    &batt_prop);
	if (!ret)
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		...
		bq_dbg(PR_DEBUG,
		       "POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT: %d\n",
		       val->intval);
		break;
					...
					const union power_supply_propval *val)
{
	...
	switch (prop) {
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		vote(bq->chg_dis_votable, BQ25790_USER_VOTER, !val->intval,
		     !val->intval);
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		bq_dbg(PR_DEBUG,
		       "POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE: %d\n",
		       val->intval);
...
static int bq25790_update_charging_profile(struct bq25790 *bq)
{
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		if (bq->hvdcp_class == HVDCP3_CLASSB_27W) {
			...
			bq25790_set_input_volt_limit(bq,
						     BQ25790_HVDCP_B_VINDPM_MV);
		} else {
			iindpm = BQ25790_HVDCP_A_IINDPM_MA;
			fcc = BQ25790_HVDCP_A_FCC;
			bq25790_set_input_volt_limit(bq,
						     BQ25790_HVDCP_A_VINDPM_MV);
		}
		break;
...
static int bq25790_psy_register(struct bq25790 *bq)
{
	...
	bq->bbc_psy =
		power_supply_register(bq->dev, &bq->bbc_psy_d, &bbc_psy_cfg);
						      ...
						      struct bq25790 *bq)
{
	...
	ret = of_property_read_u32(np, "ti,bq25790,precharge-current",
				   &pdata->iprechg);
	if (ret)
		bq_dbg(PR_OEM,
		       "Failed to read node of ti,bq25790,precharge-current\n");

	ret = of_property_read_u32(np, "ti,bq25790,termi-curr", &pdata->iterm);
	if (ret)
		bq_dbg(PR_OEM,
		       "Failed to read node of ti,bq25790,termi-curr\n");

	ret = of_property_read_u32(np, "ti,bq25790,safe_timer_en",
				   &pdata->safe_timer_en);
	if (ret)
		bq_dbg(PR_OEM,
		       "Failed to read node of ti,bq25790,safe_timer_en\n");

	ret = of_property_read_u32(np, "ti,bq25790,safe_timer",
				   &pdata->safe_timer);
	if (ret)
		bq_dbg(PR_OEM,
		       "Failed to read node of ti,bq25790,safe_timer\n");

	ret = of_property_read_u32(np, "ti,bq25790,presafe_timer",
				   &pdata->presafe_timer);
	if (ret)
		bq_dbg(PR_OEM,
		       "Failed to read node of ti,bq25790,presafe_timer\n");
	...
	if ((!gpio_is_valid(irq_gpio))) {
		bq_dbg(PR_OEM,
		       "Failed to read node of ti,bq25790,boost-current\n");

	} else {
		ret = gpio_request(irq_gpio, "bq25790_irq_gpio");
		if (ret) {
			bq_dbg(PR_OEM,
			       "%s: unable to request bq25790 irq gpio [%d]\n",
			       __func__, irq_gpio);
		}
		ret = gpio_direction_input(irq_gpio);
		if (ret) {
			bq_dbg(PR_OEM,
			       "%s: unable to set direction for bq25790 irq gpio [%d]\n",
			       __func__, irq_gpio);
		}
	...
	if ((!gpio_is_valid(bq->arti_vbus_gpio))) {
		...
		if (ret) {
			bq_dbg(PR_OEM,
			       "%s: unable to request bq25790 vbus gpio [%d]\n",
			       __func__, bq->arti_vbus_gpio);
		}
		ret = gpio_direction_output(bq->arti_vbus_gpio, 0);
		if (ret) {
			bq_dbg(PR_OEM,
			       "%s: unable to set direction for bq25790 arti vbus gpio [%d]\n",
			       __func__, bq->arti_vbus_gpio);
		}
	}
	...
	if ((!gpio_is_valid(bq->reverse_gpio))) {
		...
		if (ret) {
			bq_dbg(PR_OEM,
			       "%s: unable to request bq25790 reverse gpio [%d]\n",
			       __func__, bq->reverse_gpio);
		}
		ret = gpio_direction_output(bq->reverse_gpio, 0);
		if (ret) {
			bq_dbg(PR_OEM,
			       "%s: unable to set direction for bq25790 reverse gpio [%d]\n",
			       __func__, bq->reverse_gpio);
		}
	}
	bq_dbg(PR_OEM, "irq_gpio:%d, arti_vbus_gpio:%d, reverse:%d\n", irq_gpio,
	       bq->arti_vbus_gpio, bq->reverse_gpio);
...
static int bq25790_init_device(struct bq25790 *bq)
{
	...
	ret = bq25790_set_pre_safety_timer(bq,
					   bq->platform_data->presafe_timer);
	if (ret)
		bq_dbg(PR_OEM, "Failed to set safety timer, ret = %d\n", ret);
...
static int bq25790_detect_device(struct bq25790 *bq)
{
	...
	bq_dbg(PR_OEM,
	       "%s:BQ25790_REG_PART_NUM:0x%x bq->part_no:0x%x bq->revision:%x",
...
static void bq25790_update_status(struct bq25790 *bq)
{
	...
	bq_dbg(PR_OEM, "vbus:%d,vbat:%d, ibus:%d,ichg:%d\n", bq->vbus_volt,
	       bq->vbat_volt, bq->ibus_curr, bq->ichg_curr);
}
...
static void bq25790_fv_check(struct bq25790 *bq)
{
	...
	bq25790_get_property_from_bms(bq, POWER_SUPPLY_PROP_VOLTAGE_NOW, &prop);
	vcell = prop.intval;
	...
	bq_dbg(PR_OEM, "vcell:%d, fv:%d, fv_max:%d\n", vcell,
	       get_effective_result(bq->fv_votable), fv_max);
}

static void bq25790_status_change_work(struct work_struct *work)
{
	struct bq25790 *bq = container_of(work, struct bq25790,
					  charge_status_change_work.work);
	union power_supply_propval prop = {
		0,
	};
	...
	if (charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) {
		ret = power_supply_get_property(
			bq->usb_psy, POWER_SUPPLY_PROP_HVDCP3_TYPE, &prop);
		if (ret < 0) {
			bq_dbg(PR_OEM,
			       "couldn't read USB TYPE property, ret=%d\n",
			       ret);
			return;
		}
	...
	if ((bq->charger_type != charger_type) ||
...
u8 bq25790_word_reg[15] = { 0x01, 0x03, 0x06, 0x31, 0x33, 0x35, 0x37, 0x39,
			    0x3b, 0x3d, 0x3f, 0x40, 0x41, 0x43, 0x45 };
...
{
	struct bq25790 *bq =
		container_of(work, struct bq25790, charge_monitor_work.work);
...
{
	struct bq25790 *bq =
		container_of(work, struct bq25790, charge_irq_work.work);
	u8 status;
	int ret;
	union power_supply_propval prop = {
		0,
	...
	if (!bq->power_good) {
		...
		if (bq->wls_psy) {
			...
			bq_dbg(PR_OEM, "usb plug-out, notice wireless = %d\n",
			       bq->usb_present);
			power_supply_set_property(bq->wls_psy,
...
static int bq25790_awake_vote_cb(struct votable *votable, void *data, int awake,
				 const char *client)
{
...
static int bq25790_arti_vbus_dis_vote_callback(struct votable *votable,
					       void *data, int disable,
					       const char *client)
...
static int bq25790_create_votable(struct bq25790 *bq)
{
	...
	bq->chg_dis_votable =
		create_votable("BBC_DISABLE", VOTE_SET_ANY,
			       bq25790_chg_disable_vote_callback, bq);
	if (IS_ERR(bq->chg_dis_votable)) {
		rc = PTR_ERR(bq->chg_dis_votable);
		bq->chg_dis_votable = NULL;
	}

	bq->arti_vbus_dis_votable =
		create_votable("ARTI_VBUS", VOTE_SET_ANY,
			       bq25790_arti_vbus_dis_vote_callback, bq);
	...
	bq->awake_votable = create_votable("BBC_AWAKE", VOTE_SET_ANY,
					   bq25790_awake_vote_cb, bq);
				  ...
				  const char *buf, size_t count)
{
	...
	vote_override(bq->fcc_votable, BQ25790_MTBF_VOTER, bq->mtbf,
		      BQ25790_CDP_FCC);
	vote_override(bq->ilim_votable, BQ25790_MTBF_VOTER, bq->mtbf,
		      BQ25790_CDP_IINDPM_MA);

	return count;
}
static DEVICE_ATTR(mtbf, S_IRUGO | S_IWUSR, bq25790_show_mtbf,
		   bq25790_store_mtbf);
...
static DEVICE_ATTR(registers, S_IRUGO | S_IWUSR, bq25790_show_registers,
...
static int bq25790_notifier_call(struct notifier_block *nb, unsigned long ev,
				 void *v)
{
				 ...
				 const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&bq->charge_irq_work, bq25790_charge_irq_workfunc);
	...
	schedule_delayed_work(&bq->charge_irq_work, 0);
	bq_dbg(PR_OEM,
	       "bq25790 probe successfully, Part Num:%d, Revision:0x%x \n",
